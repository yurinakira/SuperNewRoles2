    ...
    static class ExtendedMeetingHud
    {
        public static Dictionary<byte, int> CustomCalculateVotes(this MeetingHud __instance)
        {
            ...
            for (int i = 0; i < __instance.playerStates.Length; i++)
            {
                ...
                if (!ps.AmDead && ps.VotedFor != (byte)253 && ps.VotedFor != (byte)252 && ps.VotedFor != byte.MaxValue && ps.VotedFor != (byte)254 && ModHelpers.playerById(ps.TargetPlayerId).isAlive() && ModHelpers.playerById(ps.TargetPlayerId).IsPlayer())
                {
                    int num;
                    int VoteNum = 1;
